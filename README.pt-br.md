...
> Defendemos que a cultura dos jogos deve ser inclusiva e acessível, não apenas um privilégio para quem pode pagar.
...
1. **Priorize Issues de Baixa Prioridade**:
   - Concentre-se em corrigir problemas complexos de skins (marcados como baixa prioridade)
   - Envie pull requests com soluções funcionais
   - Ajude com problemas que o ritoskin não consegue lidar automaticamente

2. **Qualidade sobre Quantidade**:
   - Consertar um único problema complexo de skin (como Exalted Jinx) conta mais do que correções simples de textura
   - Ajude com problemas que são desafiadores de resolver
   - Contribua para a saúde geral do repositório

3. **Seja Ativo na Comunidade**:
...
<details>
<summary><strong>Clique para expandir a estrutura do repositório</strong></summary>

```
/
├── Champion1/  # Pastas de campeões organizadas por ID ou nome
│   ├── 0.zip   # Skin padrão
│   ├── 1.zip 
│   ├── 2.zip
│   └── ...
...
Cada pasta de campeão contém arquivos de skin em formato `.zip`, organizados por ID de skin.
</details>
...
3. **Importe as Skins para Sua Ferramenta de Injeção:**

   <details>
   <summary>Processo de Importação Típico</summary>
   
   - A maioria das ferramentas de injeção de skins permite funcionalidade de arrastar e soltar
   - Importe os arquivos zip de skins baixados para sua ferramenta escolhida
   - Ative as skins através da interface da ferramenta
   </details>

   <details>
   <summary>Método de Importação Manual</summary>
   
   - Se arrastar e soltar não funcionar, muitas ferramentas têm uma pasta "installed"
   - Copie os arquivos de skin diretamente para essa pasta
   - Verifique a documentação da ferramenta para instruções específicas
   </details>
...
<details>
<summary><strong>Skins Brancas ou com Bugs</strong></summary>

**Problema**: Skins aparecem totalmente brancas ou têm bugs visuais

**Soluções**:
1. Baixe novamente a versão mais recente da skin do repositório
   ```bash
   # Exclua o arquivo de skin antigo da sua ferramenta de injeção primeiro
   # Baixe o arquivo de skin atualizado
   # Reimporte a nova versão
   ```
2. Verifique se você está usando a versão compatível com o patch atual
3. Verifique se o problema já foi relatado no GitHub
</details>

<details>
<summary><strong>Barras de Vida ou Áudio Ausentes</strong></summary>

**Problema**: Barras de vida estão ausentes ou não há som/vozes

**Soluções**:
1. Baixe novamente a versão atualizada das skins
2. Verifique se você está usando uma versão desatualizada
3. Reporte o problema em nosso rastreador de problemas do GitHub se os problemas persistirem
</details>

<details>
<summary><strong>Problemas de Instalação</strong></summary>

**Problema**: Sua ferramenta de injeção diz "skin injetada com sucesso", mas as skins não aparecem no jogo

**Soluções**:
1. Execute sua ferramenta de injeção como administrador
   ```powershell
   # Clique com o botão direito no executável
   # Selecione "Executar como administrador"
   ```
2. Confirme se você está usando a skin padrão para o campeão
3. Baixe novamente os arquivos de skin mais recentes
4. Verifique se você está seguindo o tutorial adequado
</details>
...
Envie problemas para nosso [rastreador de problemas do GitHub](https://github.com/darkseal-org/lol-skins/issues/new?template=custom.md)
...
Skins personalizadas são permitidas sob os termos de serviço da Riot e geralmente não acionam detecção, desde que você não discuta ou anuncie o uso das skins dentro do jogo.
